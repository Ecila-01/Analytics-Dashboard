...
export default function InsightBanner({ title, insightText, theme }: InsightBannerProps) {
  ...
  return (
    <div
      className={`bg-gradient-to-r p-5 rounded-2xl shadow-sm border flex items-start gap-4 transition-colors duration-300 ${s.wrapperBg}`}
    >
      <div className={`p-2 rounded-lg mt-0.5 flex-shrink-0 ${s.iconBg} ${s.iconColor}`}>
        <Sparkles className="w-5 h-5" />
      </div>
      <div>
        <h4 className={`text-xs font-bold uppercase tracking-wider mb-1 ${s.titleColor}`}>
          {title}
        </h4>
